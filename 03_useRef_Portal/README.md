...
- How do we read the player's input value on submit without re-rendering on every keystroke?
- How do we store the interval ID so we can clear it on stop, without triggering a re-render?
- The game tracks several values across renders — how do we decide which ones belong in state and which in a ref?
- How do we render the result modal outside the game's DOM hierarchy to avoid CSS constraints?

---

## 2. useRef Mental Model

Before examining design decisions, build a correct mental model of `useRef`. Every design decision in this project follows directly from these three principles.
...
console.log(countRef.current); // 1, always the latest value
```
...
React sets `inputRef.current` to the DOM element after mount, and back to `null` on unmount. Use for: focus, scroll, measure, integrating third-party libraries that need direct DOM access (charts, maps, canvas, video players).
...
|---|---|
| Persistent mutable box (2.1) | Survives re-renders; changing it doesn't cause one; always reads the latest value |
| Two use cases (2.2) | DOM node reference, or mutable value storage |
| Not in render (2.3) | Only read/write inside event handlers or `useEffect` |
...
*Principles: [2.1](#21-useref--a-persistent-mutable-box-not-a-snapshot) — persistent mutable box, [2.2](#22-two-main-use-cases) — DOM node reference*
...
*Principles: [2.1](#21-useref--a-persistent-mutable-box-not-a-snapshot) — persistent mutable box, [2.2](#22-two-main-use-cases) — mutable value storage, [2.3](#23-never-read-or-write-refcurrent-during-render) — not in render*
...
*Principle: [2.2](#22-two-main-use-cases) — DOM node reference*
...
| Two use cases (2.2) | Interval ID: ref not state (3.2) | TypeScript value refs (4) |
| Not in render (2.3) | Result modal: Portal (3.3) | |
